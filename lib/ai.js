import { PROVIDER_SETTINGS, HEALTH_SETTINGS, API_URLS, DEFAULT_IMAGE_MODEL } from "../config.js";
...
export async function completeWithAIStream(prompt, messages, model, onChunk) {
    ...
    switch (route) {
        case "deepai":
            const result = await vexaCompleteStream(prompt, messages, model, onChunk);
            return result;
        case "pollinations":
            await pollinationsCompleteStream(messages, model, onChunk);
            break;
        case "talkai":
            await talkaiCompleteStream(prompt, model, onChunk);
            break;
        case "dolphin":
            await dolphinCompleteStream(prompt, model, onChunk);
            break;
        case "aifree":
            await aiFreeCompleteStream(prompt, messages, onChunk);
            break;
        case "toolbaz":
            await toolbazCompleteStream(prompt, model, onChunk);
            break;
        default:
...
export async function generateImage(prompt, model, preference) {
    ...
    return await callDeepAIImage(String(prompt).trim().slice(0, 1000), DEFAULT_IMAGE_MODEL, prefKey);
}
