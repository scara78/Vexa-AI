...
import { resolveModelRoute, resolveDeepAIModel, resolveImageRoute, resolveImagePrefKey, validImageModels, getTalkaiModelIds, refreshModelsCache } from "./models.js";
...
const { DOLPHIN_MODELS, POLLINATIONS_MODELS, DEEPAI_MODELS } = MODEL_SETS;
...
export async function checkModel(model) {
    const t0 = Date.now();
    try {
        const talkaiIds = getTalkaiModelIds();
        if (talkaiIds.has(model) && !PROVIDER_SETTINGS.talkai) return { ok: false, error: "Provider disabled", latency_ms: Date.now() - t0 };
        ...
        let text;
        if (talkaiIds.has(model)) text = await talkaiComplete(HEALTH_PROBE, model);
