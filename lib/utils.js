...
export function cleanText(text) {
    let cleaned = text.replace(/^(Claude \d+(\.\d+)?\s+(Sonnet|Haiku|Opus)|GPT-\d+(\.\d+)?|Gemini \w+)\s+/i, '');
    cleaned = cleaned.replace(/\s*-\d+$/, '');
    return cleaned;
}

export function cleanExcessiveBrTags(text) {
    return text
        .replace(/<br>\s*<br>\s*<br>\s*<br>/g, '</p><p>')
        .replace(/<br>\s*<br>\s*<br>/g, '</p><p>')
