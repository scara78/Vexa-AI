...
import json, time, collections, base64, random, re, html as html_lib
...
def _toolbaz_complete(prompt: str, model: str) -> str:
    ...
    r = s.post(TOKEN_URL, data={"session_id": SESSION_ID, "token": client_token}, headers=HDRS, timeout=10)
    r.raise_for_status()
    capcha = r.json().get("token", "")
    if not capcha:
        raise ValueError("Failed to obtain capcha token")
    r2 = s.post(WRITE_URL, data={
        "text": prompt, "capcha": capcha, "model": model, "session_id": SESSION_ID,
